...
class M2DP: 
    ...
    def GetSignatureMatrix(self,azimuthList,elevationList, cloud_pca, 
                        numT, numR, maxRho):
        ...
        for azm in azimuthList:
            ...
            for elv in elevationList:
                ...
                rho,theta=self.cart2pol(pcx,pcy)
    ...
    def runM2DP(self,cloud):
        ...
        half_pi=math.pi/2
        azimuthList =np.linspace(-half_pi,half_pi,self.numP)
        elevationList = np.linspace(0,half_pi,self.numQ)
        ...
        A=self.GetSignatureMatrix(azimuthList, elevationList, cloud_pca, self.numT, self.numR, maxRho)
        #run SVD on A and use [u0,v0] as the final output
        u,s,v = np.linalg.svd(A)
        desM2DP = np.append(u[:,0],v.T[:,0])
        # print(f"time used for calculating a point cloud with M2DP: {time.time() - start_time:.3f} seconds")
        return desM2DP,A
